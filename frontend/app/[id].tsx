...
import { ScrollView, StyleSheet, ActivityIndicator, Image, View, Alert, TouchableOpacity, useColorScheme, Dimensions, Modal, Platform } from 'react-native';
  ...
  const fetchReport = async () => {
    try {
      ...
      if (data.image_paths?.length > 0) {
        const { data: signedData } = await supabase.storage.from('report-photos').createSignedUrls(data.image_paths, 3600);
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: theme.tint + '15' }]}>
            ...
            <ThemedText style={[styles.statusText, { color: theme.tint }]}>{report?.status?.toUpperCase() || 'PENDING'}</ThemedText>
          </View>

          <ThemedText type="title" style={styles.mainTitle}>{report?.title || "Issue Report"}</ThemedText>
          <ThemedText style={styles.dateText}>Reported on {report?.created_at ? new Date(report.created_at).toLocaleDateString() : '...'}</ThemedText>
          ...
          <View style={[styles.card, { borderColor: theme.icon + '20' }]}>
            ...
            <View style={styles.locationRow}>
              ...
              <ThemedText style={styles.locationValue}>{renderLocation(report?.location)}</ThemedText>
            </View>
          </View>
          ...
          <View style={styles.imageSection}>
            <View style={styles.sectionHeader}>
              ...
              {imageUrls.length > 1 && (
                <ThemedText style={styles.imageCount}>{activeIndex + 1} of {imageUrls.length}</ThemedText>
              )}
            </View>

            <ScrollView 
              horizontal pagingEnabled showsHorizontalScrollIndicator={false} 
              onMomentumScrollEnd={(e) => setActiveIndex(Math.round(e.nativeEvent.contentOffset.x / (width - 40)))}
              scrollEventThrottle={16}
            >
              {imageUrls.map((url, index) => (
                <TouchableOpacity key={index} activeOpacity={0.9} onPress={() => openViewer(index)} style={styles.imageWrapper}>
            ...
            {imageUrls.length > 1 && (
              <View style={styles.paginationContainer}>
                {imageUrls.map((_, i) => (
                  <View key={i} style={[styles.paginationDot, { backgroundColor: i === activeIndex ? theme.tint : theme.icon + '30' }]} />
                ))}
              </View>
            )}
          </View>
      ...
      <Modal visible={isModalVisible} transparent animationType="fade" statusBarTranslucent>
        <View style={styles.modalBackground}>
          <View style={styles.modalHeaderContainer}>
            <ThemedText style={styles.modalCount}>{viewerIndex + 1} / {imageUrls.length}</ThemedText>
            <TouchableOpacity onPress={() => setIsModalVisible(false)} style={styles.closeButton} hitSlop={20}>
          ...
          <View style={styles.modalMainContainer}>
            ...
            {/* Navigation Buttons */}
            {viewerIndex > 0 && (
              <TouchableOpacity style={[styles.navButton, styles.leftButton]} onPress={() => scrollToIndex(viewerIndex - 1)}>
                <Ionicons name="chevron-back" size={36} color="#FFF" />
              </TouchableOpacity>
            )}
            {viewerIndex < imageUrls.length - 1 && (
              <TouchableOpacity style={[styles.navButton, styles.rightButton]} onPress={() => scrollToIndex(viewerIndex + 1)}>
